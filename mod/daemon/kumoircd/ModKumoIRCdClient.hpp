...
class ModKumoIRCdClient: public ClientTcpLine {
	Q_OBJECT;
public:
	ModKumoIRCdClient(QTcpSocket *sock, ModKumoIRCd *parent);
...
public slots:
	void irc_cmd_nick(const QList<QByteArray>&, const QByteArray &prefix);

protected:
	void handleLine(const QByteArray&);
	void handleIrcCommand(const QList<QByteArray>&, const QByteArray &prefix);

	void rawReply(enum irc_raw raw, const QList<QByteArray>&reply);
	void rawReply(enum irc_raw raw, const QList<QByteArray>&reply, bool last_with_space);
	void sendReply(const QList<QByteArray>&, const QByteArray &prefix, bool last_param_space = false);
